...
argument-hint: [<dir | file | file:func> | (空=リポジトリ全体)] [: 特に知りたいこと]
---
...
- **進め方は解説型**: Claude が各層を読み解いて全体像・図・コード解説を提示する。ユーザーは読んで理解する。
...
`$ARGUMENTS` の先頭トークンを対象パス、`:` 以降（または残りの自然文）を「特に知りたいこと（フォーカス）」とみなす。
...
2. 地図を作るために、軽く眺める（`Glob` / `Read`、`Bash` の `git ls-files` 等）:
   - ディレクトリツリー（深さ2〜3程度）、エントリポイント候補
   - マニフェスト（`pyproject.toml` / `package.json` / `go.mod` / `Cargo.toml` 等）と主要依存
   - `README` / `docs/` / `CLAUDE.md`（あれば。プロダクトの意図が早く掴める）
...
- 言語バージョン・依存・仮想環境などの前提は、各リポジトリの `CLAUDE.md` / `docs/architecture.md` / `README` を参照する。
